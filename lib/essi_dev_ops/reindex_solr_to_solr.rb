module EssiDevOps
  class ReindexSolrToSolr
    ...
    class SolrDocReconstructor
      ...
      # ESSI::CampusMetadata
      CAMPUS_METADATA_FIELDS = {
        "campus_sim" => "campus_tesim"
      }

      def reconstruct_dynamic_fields(new_doc)
        return unless new_doc["dynamic_schema_id_ssi"].present?
        schema = AllinsonFlex::DynamicSchema.find(new_doc["dynamic_schema_id_ssi"])
        problem_fields = []
        schema.schema["properties"].each do |field, field_def|
          non_stored = field_def["indexing"] & NON_STORED_DEFINITIONS
          next unless non_stored.present?
          stored = field_def["indexing"] & STORED_DEFINITIONS
      ...
      def reconstruct_includes(new_doc, klass)
        ...
        CAMPUS_METADATA_FIELDS.each { |unstored, stored| new_doc[unstored] = new_doc[stored] } if klass.ancestors.include? ESSI::CampusMetadata
        new_doc["iiif_index_strategy_tesim"] = ["iiif_print_v1.0"] if klass.ancestors.include? ESSI::OCRBehavior
        reconstruct_dynamic_fields(new_doc) if klass.ancestors.include? AllinsonFlex::DynamicMetadataBehavior
        new_doc
      end
