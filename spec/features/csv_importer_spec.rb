...
RSpec.feature 'Create and run a CSV Importer', type: :system, js: true do
  context 'a logged in user', clean: true do
    ...
    scenario do
      ...
      expect(page).to have_content 'File required'
      ...
      # expect(page).to have_field 'selected_files[0][file_name]', with: 'rbg.png'
      # expect(page).to have_field 'selected_files[1][file_name]', with: 'world.png'
      # expect(find('#cloud-files')).to have_content 'spec/fixtures/rgb.png'
      ...
      click_link 'Image'
      # On work show page
      expect(page).to have_content 'P. Tikas 3 recto'
      expect(page).to have_content 'Indiana University East'
